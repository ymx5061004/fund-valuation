...
- **/watchlist（自选）** = `WatchlistView`。读写同一份 `localStorage('fv.watchlist')`（与 /market 的 ★ 同步）。列表显示**当日涨幅**(实时估值 estimateChangePct) + **当日估值**(estimateNav)，每 20s 刷新；可搜索添加、点 ★ 移除、按当日涨幅排序。
  注：原型的「关联板块」是养基宝自建的板块标签，天天基金接口拿不到，故按用户要求第二列改显「当日估值」。当日涨幅用的是**估值涨幅**（盘中即此；收盘后与官方确认涨幅会略有差）。
